<template>
    <div class="main">
        ...
        <div class="container" v-if="state.list.length">
            ...
            <div class="box">
                ...
                <div class="item" v-for="item in state.list" :key="item">
                    <span> {{ item.one }}{{ state.currentTime.value }}观察记录包 </span>
                    <a @click="hanldehange(val, item)" v-for="val of state.tab" :key="val.id">{{
                        val.name
                    }}</a>
                    <el-divider />
                </div>
        ...
        <Dialog :btn-text="state.btnText" :dialog-visible="state.show" :message="state.message" />
    </div>
</template>
...
// 整包下载  预览  自定义下载
const hanldehange = (val: { id:number }, item: any) => {
    console.log('点击了', val, '值', item)
